-- Modules Schema for tracking student modules
-- Run this script to create the Modules schema

IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Modules]') AND type in (N'U'))
BEGIN
    CREATE SCHEMA [dbo].[Modules] (
    ...
    CREATE NONCLUSTERED INDEX [IX_Interventions_StudentID] ON [dbo].[Modules]([StudentID]);
    CREATE NONCLUSTERED INDEX [IX_Interventions_Status] ON [dbo].[Modules]([Status]);
    
    PRINT 'Modules schema created successfully';
END
ELSE
BEGIN
    PRINT 'Modules schema already exists';
END
GO

-- Insert sample intervention data
INSERT INTO [dbo].[Modules] ([StudentID], [InterventionType], [Description], [Status], [Priority], [AssignedTo])
SELECT 3, 'Academic Support', 'Additional tutoring sessions for struggling subjects', 'Active', 'High', 'Dr. Sarah Williams'
WHERE NOT EXISTS (SELECT 1 FROM Modules WHERE StudentID = 3 AND InterventionType = 'Academic Support');

INSERT INTO [dbo].[Modules] ([StudentID], [InterventionType], [Description], [Status], [Priority], [AssignedTo])
SELECT 1, 'Counseling', 'Weekly counseling sessions to address stress and anxiety', 'Active', 'Critical', 'Dr. James Mokoena'
WHERE NOT EXISTS (SELECT 1 FROM Modules WHERE StudentID = 1 AND InterventionType = 'Counseling');
